...
export const walletDepositSchema = z
  .object({
    ...
    cardProvider: z.string().trim().min(2).max(40).optional(),
    cardSerial: z.string().trim().min(3).max(100).optional(),
    cardCode: z.string().trim().min(4).max(200).optional(),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.method === "CARD") {
      if (!data.cardProvider) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Vui lòng chọn loại thẻ.", path: ["cardProvider"] });
      if (!data.cardSerial) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Vui lòng nhập số seri thẻ.", path: ["cardSerial"] });
      if (!data.cardCode) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Vui lòng nhập mã thẻ cào.", path: ["cardCode"] });
    }
  });
