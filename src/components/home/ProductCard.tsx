...
import { CheckCircle2, Download, ArrowUpRight, Sparkles, Zap, ShoppingCart, Eye } from "lucide-react";
...
export function ProductCard({product}:{product:ProductCardData}){
  const router=useRouter(); const addToCart=useAddToCart(); const {data:userData}=useCurrentUser(); const {show}=useToast(); const {t}=useTranslation(); const discountValue=product.discountPrice!=null&&Number(product.discountPrice)<Number(product.price)?Number(product.discountPrice):null;
  ...
  // Subtle pointer-tilt (desktop only) — the premium "3D product card"
  // effect seen on high-end e-commerce sites. Deliberately restrained
  // (±5deg) and layered on top of the existing whileHover lift via
  // Framer Motion's own motion-value props (rotateX/rotateY on `style`),
  // never a raw `style={{ transform: "..." }}` string — that pattern is
  // exactly what silently broke the Vault Core 3D centering earlier this
  // project (a hand-written transform string overwrites, rather than
  // composes with, any other transform Framer Motion or Tailwind sets on
  // the same element). Framer Motion's dedicated motion-value props don't
  // have that problem; it composes them into one transform internally.
  ...
  return <motion.article
    ...
    className="khv-product-card group glass-surface khv-card-shine khv-hover-glow relative flex h-full min-h-[610px] flex-col overflow-hidden rounded-[30px] border-white/[.085] shadow-[0_24px_90px_rgba(0,0,0,.26)]">
    <Link href={`/san-pham/${product.slug}`} className="block" aria-label={`Xem ${product.name}`} onClick={() => {
      ...
      <div className="absolute left-4 top-4 flex flex-wrap gap-2">{product.isFeatured&&<span className="inline-flex items-center gap-1.5 rounded-full border border-accent-orange/35 bg-black/50 px-3 py-1.5 text-[10px] font-bold tracking-[.14em] text-accent-orange backdrop-blur-xl"><Sparkles className="h-3 w-3"/> {t("NỔI BẬT")}</span>}{product.isVipOnly&&<span className="rounded-full border border-accent-blue/35 bg-black/50 px-3 py-1.5 text-[10px] font-bold tracking-[.14em] text-accent-blue backdrop-blur-xl">VIP</span>}{hasDiscount&&<span className="rounded-full border border-state-success/30 bg-black/50 px-3 py-1.5 text-[10px] font-bold tracking-[.14em] text-state-success backdrop-blur-xl">{t("ƯU ĐÃI")}</span>}</div>
      <button
        type="button"
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); setQuickViewOpen(true); }}
        className="khv-touch-target absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-black/40 text-white/80 opacity-0 backdrop-blur-xl transition-all hover:bg-black/60 hover:text-white group-hover:opacity-100 sm:opacity-0"
        aria-label={`Xem nhanh ${product.name}`}
        title="Xem nhanh — không cần rời trang"
      >
        <Eye className="h-4 w-4" />
      </button>
      <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3"><span className="rounded-full border border-white/10 bg-black/30 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[.16em] text-white/65 backdrop-blur-xl">{product.category?.name??t("File hỗ trợ")}</span>{typeof product.salesCount==="number"&&product.salesCount>0&&<span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-black/35 px-2.5 py-1 text-[10px] text-white/65 backdrop-blur-xl"><Download className="h-3 w-3"/>{product.salesCount} {t("đã bán")}</span>}</div>
    </div></Link>
    <div className="flex flex-1 flex-col p-5 sm:p-7"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><h3 className="text-[24px] font-semibold leading-[1.12] tracking-[-.025em] text-white transition-colors group-hover:text-accent-orange">{product.name}</h3><div className="mt-2 flex flex-wrap items-center gap-1.5">{(product.reviewCount??0)>0?<>{Array.from({length:5}).map((_,i)=><span key={i} className={cn("text-[14px] leading-none",i<Math.round(product.averageRating??0)?"text-accent-orange":"text-white/15")}>★</span>)}<span className="text-[10px] text-white/35">{(product.averageRating??0).toFixed(1)} · {product.reviewCount} {t("đánh giá")}</span></>:<span className="text-[10px] text-white/35">{t("Chưa có đánh giá")}</span>}</div></div><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/[.035] text-white/45 transition-all group-hover:border-accent-orange/35 group-hover:bg-accent-orange/10 group-hover:text-accent-orange"><ArrowUpRight className="h-4 w-4"/></span></div>
      ...
      <div className="grid grid-cols-2 gap-2.5"><button type="button" onClick={(e)=>{e.preventDefault();e.stopPropagation();setQuickViewOpen(true);}} className="khv-touch-target flex items-center justify-center gap-1.5 rounded-2xl border border-white/[.08] bg-white/[.025] px-3 py-3 text-[11px] font-semibold uppercase tracking-[.12em] text-white/60 transition-all hover:border-accent-orange/25 hover:text-white"><Eye className="h-3.5 w-3.5"/> {t("Xem nhanh")}</button><button type="button" onClick={e=>{e.preventDefault();e.stopPropagation();if(!userData?.user){show("Vui lòng đăng nhập để mua sản phẩm.","info");router.push("/dang-nhap");return;}addToCart.mutate({productId:product.id ?? ""}, {onSuccess:()=>router.push("/thanh-toan"),onError:(err)=>show(err instanceof ApiError?err.message:"Không thể thêm sản phẩm vào giỏ hàng.","error")});}} className="khv-touch-target khv-interactive flex items-center justify-center gap-2 rounded-2xl border border-accent-orange/35 bg-gradient-to-r from-accent-orange to-[#ff9f5c] px-3 py-3 text-[11px] font-bold uppercase tracking-[.10em] text-black shadow-[0_10px_35px_rgba(255,138,61,.18)] hover:shadow-[0_14px_45px_rgba(255,138,61,.30)]"><ShoppingCart className="h-3.5 w-3.5"/> {t("Mua ngay")}</button></div></div>
    </div>
