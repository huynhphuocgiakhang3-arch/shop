...
function Bubble({ message, adminName, adminAvatar }: { message: ChatMessageItem; adminName: string; adminAvatar: string | null }) {
  ...
  return (
    <div className={cn("flex items-end gap-2", isUser ? "justify-end" : "justify-start")}>
      ...
      <div
        className={cn(
          "max-w-[75%] whitespace-pre-line rounded-2xl px-3.5 py-2.5 text-small shadow-sm",
...
export function ChatPanel({ onClose }: { onClose: () => void }) {
  return (
    <motion.div
      ...
      className="glass-surface absolute bottom-16 right-0 flex w-80 flex-col overflow-hidden rounded-lg border border-white/10 shadow-2xl backdrop-blur-2xl"
    >
