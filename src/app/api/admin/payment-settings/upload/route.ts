...
export async function POST(req: NextRequest) {
  ...
  if (contentType.includes("application/json")) {
    ...
    const settings = await updatePaymentSettings({ [field]: body.url }, user.sub);
    await prisma.auditLog.create({ data: { userId: user.sub, action: "SUPER_ADMIN_UPLOAD_PAYMENT_ASSET", metadata: { target: body.target, url: body.url, publicId: body.publicId } } });
