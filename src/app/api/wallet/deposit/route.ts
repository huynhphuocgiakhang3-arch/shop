...
export async function POST(req: NextRequest) {
  try {
    ...
    if (parsed.data.cardCode) {
      const dup = await prisma.depositRequest.findFirst({ where: { cardCode: parsed.data.cardCode } });
      if (dup) return jsonError("Mã thẻ này đã được sử dụng cho một yêu cầu khác.", 409);
    }
