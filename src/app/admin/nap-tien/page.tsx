          ...
          {items.map((item) => (
            <GlassPanel key={item.id} radius="md" className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                {item.method === "CARD" && (
                  <div className="flex min-w-[220px] flex-col gap-1 rounded-md border border-white/10 bg-white/[0.03] px-3 py-2">
                    <span className="text-caption font-medium text-accent-orange/90">{item.cardProvider ?? "Thẻ cào"}</span>
                    <code className="text-caption text-white/65">Seri: {item.cardSerial ?? "—"}</code>
                    <code className="text-caption text-white/75">Mã: {item.cardCode ?? "—"}</code>
                  </div>
                )}
