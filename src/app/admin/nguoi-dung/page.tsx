...
import { formatDate, MEMBERSHIP_LABEL, formatVnd } from "@/lib/format";
          ...
          <table className="w-full text-left text-small">
            <thead>
              <tr className="border-b border-white/10 text-caption text-white/40">
                ...
                <th className="px-5 py-3">Số dư ví</th>
            ...
            <tbody>
              {data.items.map((user) => (
                <tr key={user.id} className="border-b border-white/5">
                  ...
                  <td className="px-5 py-3"><span className="font-medium text-accent-orange">{formatVnd(Number(user.wallet?.balance ?? 0))}</span>{Number(user.wallet?.pendingBalance ?? 0) > 0 && <span className="ml-2 text-caption text-white/30">+{formatVnd(Number(user.wallet?.pendingBalance ?? 0))} chờ</span>}</td>
