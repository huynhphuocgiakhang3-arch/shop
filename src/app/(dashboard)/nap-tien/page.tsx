...
export default function DepositPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (method === "CARD" && cardCode.trim().length < 4) {
      show("Vui lòng nhập mã thẻ cào hợp lệ.", "error");
      return;
    }
            ...
            <GlassPanel radius="md" className="p-6">
              ...
              {method === "QR_BANK" ? (
                ...
                <div className="mt-5 flex flex-col gap-4">
                  ...
                  <Input label="Mã thẻ cào" value={cardCode} onChange={(e) => setCardCode(e.target.value)} placeholder="Nhập seri + mã thẻ" />
                  <Button onClick={handleSubmit} isLoading={deposit.isPending} disabled={!cardCode || !amount}>
                    Gửi yêu cầu nạp thẻ
                  </Button>
                </div>
              )}
