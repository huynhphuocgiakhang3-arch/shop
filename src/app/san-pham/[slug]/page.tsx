        ...
        <main className="mx-auto flex w-full max-w-lg flex-1 flex-col items-center justify-center px-4 py-16 text-center">
          <GlassPanel radius="xl" className="flex flex-col items-center gap-4 p-8 sm:p-10">
            ...
            <div className="mt-2 flex w-full flex-col gap-3 sm:flex-row sm:justify-center">
              <Button variant="primary" className="khv-touch-target w-full sm:w-auto" onClick={() => router.push("/san-pham")}>
                Xem sản phẩm khác
              </Button>
              <Button variant="secondary" className="khv-touch-target w-full sm:w-auto" onClick={() => router.push("/")}>
                Về trang chủ
              </Button>
            </div>
          </GlassPanel>
              ...
              {hasPurchased ? (
                ...
                <>
                  ...
                  <Button variant="secondary" onClick={() => {
                    if (!userData?.user) return requireLogin();
                    addToCart.mutate({ productId: product.id }, { onSuccess: () => router.push("/thanh-toan"), onError: (err) => show(err instanceof ApiError ? err.message : "Không thể bắt đầu mua.", "error") });
