...
export function useWalletDeposit() {
  ...
  return useMutation({
    mutationFn: (input: { method: "QR_BANK" | "CARD"; amount: number; proofImageUrl?: string; cardCode?: string; note?: string }) =>
...
export function useUploadDepositProof() {
  return useMutation({
    mutationFn: async (file: File) => {
      ...
      const res = await fetch("/api/wallet/deposit/upload", { method: "POST", body: form, credentials: "include" });
      const body = (await res.json().catch(() => ({}))) as { url?: string; message?: string };
      if (!res.ok || !body.url) throw new Error(body.message ?? "Tải ảnh lên thất bại.");
      return body.url;
    }
  });
}
